...
Shader "World"
{
	Properties
	{
		...
		[Toggle(_ALPHACLIP_ON)] _AlphaClip("AlphaClip", Float) = 1
		_AlphaClipTexture("AlphaClipTexture", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			float3 AdditionalLightsHalfLambertMask14x( float3 WorldPosition, float2 ScreenUV, float3 WorldNormal, float4 ShadowMask )
			{
				...
				return Color;
					...
					float2 voronoihash247( float2 p )
					{
					...
					float voronoi247( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )
					{
						float2 n = floor( v );
						float2 f = frac( v );
						float F1 = 8.0;
				 ...
				 ) : SV_Target
			{
				...
				float4 AdditionalLight196 = ( float4( halfLambertResult58_g20 , 0.0 ) * IN.ase_color );
				float3 hsvTorgb99 = RGBToHSV( AdditionalLight196.rgb );
				...
				#ifdef _TRIPLANAR_ON
				float2 staticSwitch96 = ( _UVScale * UV41_g42 );
				#else
				float2 staticSwitch96 = ( texCoord97 * _UVScale );
				#endif
				...
				float clampResult312 = clamp( ( ( AdditionalLightLength211 * 0.001 ) + 1.0 ) , 0.0 , 40.0 );
				float LightBoost273 = clampResult312;
				float3 Light214 = ( hsvTorgb98 * LightBoost273 );
				...
				float3 hsvTorgb3_g39 = HSVToRGB( float3(( ( ( ase_objectScale.x + ase_objectScale.y + ase_objectScale.z ) * 0.1777777 ) % 1.0 ),1.0,1.0) );
				float time247 = 0.0;
				float2 voronoiSmoothId247 = 0;
				float2 coords247 = UVUV197 * 2.0;
				float2 id247 = 0;
				float2 uv247 = 0;
				float voroi247 = voronoi247( coords247, time247, id247, uv247, 0, voronoiSmoothId247 );
				float2 break248 = id247;
				float lerpResult249 = lerp( break248.x , break248.y , 0.5);
				float3 ase_worldViewDir = ( _WorldSpaceCameraPos.xyz - WorldPosition );
				ase_worldViewDir = normalize(ase_worldViewDir);
				float fresnelNdotV258 = dot( ase_worldNormal, ase_worldViewDir );
				float fresnelNode258 = ( 0.0 + 3.0 * pow( 1.0 - fresnelNdotV258, 2.0 ) );
				float dotResult255 = dot( float3(1,-1,1) , ase_worldNormal );
				float lerpResult300 = lerp( ( lerpResult249 * ( 1.0 - fresnelNode258 ) * (0.5 + (dotResult255 - -1.0) * (0.8 - 0.5) / (1.0 - -1.0)) ) , 0.5 , 0.5);
				float3 temp_cast_3 = (lerpResult300).xxx;
				float3 lerpResult297 = lerp( hsvTorgb3_g39 , temp_cast_3 , 0.5);
				float3 DEBUG251 = lerpResult297;
				#ifdef _DEBUG_ON
				float4 staticSwitch250 = float4( DEBUG251 , 0.0 );
				#else
				float4 staticSwitch250 = ( ( float4( ( Light214 * TotalDetail216 ) , 0.0 ) * _Color ) * _Emis );
				#endif
				
				float2 texCoord306 = IN.ase_texcoord5.zw * float2( 1,1 ) + float2( 0,0 );
				#ifdef _ALPHACLIP_ON
				float staticSwitch304 = tex2D( _AlphaClipTexture, texCoord306 ).r;
				#else
				float staticSwitch304 = 1.0;
				#endif
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = ( staticSwitch250 * ( IN.ase_color * IN.ase_color ) ).rgb;
				float Alpha = staticSwitch304;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 texCoord306 = IN.ase_texcoord2.xy * float2( 1,1 ) + float2( 0,0 );
				#ifdef _ALPHACLIP_ON
				float staticSwitch304 = tex2D( _AlphaClipTexture, texCoord306 ).r;
				#else
				float staticSwitch304 = 1.0;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				float2 texCoord306 = IN.ase_texcoord.xy * float2( 1,1 ) + float2( 0,0 );
				#ifdef _ALPHACLIP_ON
				float staticSwitch304 = tex2D( _AlphaClipTexture, texCoord306 ).r;
				#else
				float staticSwitch304 = 1.0;
				...
				surfaceDescription.Alpha = staticSwitch304;
		...
		Pass
		{
				 ...
				 )
			{
				...
				float2 texCoord306 = IN.ase_texcoord1.xy * float2( 1,1 ) + float2( 0,0 );
				#ifdef _ALPHACLIP_ON
				float staticSwitch304 = tex2D( _AlphaClipTexture, texCoord306 ).r;
				#else
				float staticSwitch304 = 1.0;
...
Node;AmplifyShaderEditor.SamplerNode;11;-985.6856,-5178.196;Inherit;True;Property;_Detail;Detail;8;0;Create;True;0;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.RangedFloatNode;17;-1810.062,-4978.892;Inherit;False;Property;_DetailScale;DetailScale;9;0;Create;True;0;0;0;False;0;False;1;0;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;53;3977.565,-3788.2;Inherit;False;Property;_LightStepMax;LightStepMax;21;0;Create;True;0;0;0;False;0;False;1;1;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;52;3937.564,-3892.199;Inherit;False;Property;_LightStepMin;LightStepMin;20;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.GetLocalVarNode;208;4659.943,-4322.346;Inherit;False;196;AdditionalLight;1;0;OBJECT;;False;1;COLOR;0
...
Node;AmplifyShaderEditor.RangedFloatNode;49;2595.745,-3895.44;Inherit;False;Property;_TextureLightInflu;TextureLightInflu;22;0;Create;True;0;0;0;False;0;False;0.5;0.5;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.ColorNode;100;185.5827,-7649.999;Inherit;False;Property;_Color;Color;4;0;Create;True;0;0;0;False;0;False;0,0,0,0;0,0,0,0;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.RangedFloatNode;122;752.1813,-7701.632;Inherit;False;Property;_Emis;Emis;5;0;Create;True;0;0;0;False;0;False;1;1;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.SamplerNode;13;-996.6612,-4309.919;Inherit;True;Property;_Variation;Variation;10;0;Create;True;0;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.RangedFloatNode;18;-1550.381,-4138.628;Inherit;False;Property;_VarianceScale;VarianceScale;11;0;Create;True;0;0;0;False;0;False;0.1;0;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.SamplerNode;81;-988.1376,-3437.875;Inherit;True;Property;_BigVariation;BigVariation;12;0;Create;True;0;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.RangedFloatNode;82;-1720.262,-3278.296;Inherit;False;Property;_BigVarianceScale;BigVarianceScale;13;0;Create;True;0;0;0;False;0;False;0.1;0;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;14;-1760.97,-2379.251;Inherit;False;Property;_DetailVariance;DetailVariance;7;0;Create;True;0;0;0;False;0;False;0.5;0;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;86;-1222.09,-1948.862;Inherit;False;Property;_BigVarianceFactor;BigVarianceFactor;14;0;Create;True;0;0;0;False;0;False;0.5;0;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;56;3020.545,-3025.49;Inherit;False;Property;_Ambient;Ambient;23;0;Create;True;0;0;0;False;0;False;1;0;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;243;4516.179,-5277.761;Inherit;False;2;2;0;FLOAT3;0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.VertexColorNode;245;4160.179,-5185.761;Inherit;False;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.StaticSwitch;250;1951.976,-8054.115;Inherit;False;Property;_Debug;Debug;0;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Create;True;True;All;9;1;COLOR;0,0,0,0;False;0;COLOR;0,0,0,0;False;2;COLOR;0,0,0,0;False;3;COLOR;0,0,0,0;False;4;COLOR;0,0,0,0;False;5;COLOR;0,0,0,0;False;6;COLOR;0,0,0,0;False;7;COLOR;0,0,0,0;False;8;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.DotProductOpNode;255;1983.14,-12487.63;Inherit;False;2;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;1;FLOAT;0
Node;AmplifyShaderEditor.Vector3Node;254;1647.799,-12630.49;Inherit;False;Constant;_Vector2;Vector 2;17;0;Create;True;0;0;0;False;0;False;1,-1,1;0,0,0;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.WorldNormalVector;256;1681.696,-12318.15;Inherit;False;False;1;0;FLOAT3;0,0,1;False;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;253;2656.243,-12691.01;Inherit;False;3;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.OneMinusNode;259;2521.864,-13006.99;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.TFHCRemapNode;257;2156.258,-12494.9;Inherit;False;5;0;FLOAT;0;False;1;FLOAT;-1;False;2;FLOAT;1;False;3;FLOAT;0.5;False;4;FLOAT;0.8;False;1;FLOAT;0
Node;AmplifyShaderEditor.BreakToComponentsNode;248;1776.029,-12833.51;Inherit;False;FLOAT2;1;0;FLOAT2;0,0;False;16;FLOAT;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT;5;FLOAT;6;FLOAT;7;FLOAT;8;FLOAT;9;FLOAT;10;FLOAT;11;FLOAT;12;FLOAT;13;FLOAT;14;FLOAT;15
Node;AmplifyShaderEditor.LerpOp;249;1943.029,-12824.51;Inherit;False;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0.5;False;1;FLOAT;0
Node;AmplifyShaderEditor.TFHCRemapNode;268;1925.621,-13032.53;Inherit;False;5;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;1;False;3;FLOAT;1;False;4;FLOAT;0.5;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;269;1261.621,-13146.53;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT2;0,0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.RangedFloatNode;270;1064.621,-13168.53;Inherit;False;Constant;_Float4;Float 4;17;0;Create;True;0;0;0;False;0;False;10;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;209;3867.426,-4786.565;Inherit;False;196;AdditionalLight;1;0;OBJECT;;False;1;COLOR;0
Node;AmplifyShaderEditor.LengthOpNode;43;4139.618,-4785.859;Inherit;False;1;0;COLOR;0,0,0,0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;271;4782.46,-4765.973;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleAddOpNode;274;4977.46,-4728.973;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;275;4832.46,-4637.973;Inherit;False;Constant;_Float9;Float 9;17;0;Create;True;0;0;0;False;0;False;1;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RegisterLocalVarNode;214;5888.899,-4190.876;Inherit;False;Light;-1;True;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;276;5614.673,-4123.919;Inherit;False;2;2;0;FLOAT3;0,0,0;False;1;FLOAT;0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.GetLocalVarNode;277;5410.673,-3981.919;Inherit;False;273;LightBoost;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;272;4555.46,-4660.973;Inherit;False;Constant;_Float8;Float 8;17;0;Create;True;0;0;0;False;0;False;0.001;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;278;4586.673,-4524.919;Inherit;False;Constant;_Float10;Float 10;17;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;246;990.0291,-12956.51;Inherit;False;197;UVUV;1;0;OBJECT;;False;1;FLOAT2;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;289;2345.072,-7947.114;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;291;2073,-7676.273;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.VertexColorNode;290;1833.944,-7706.979;Inherit;False;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.GetLocalVarNode;252;1624.397,-7876.149;Inherit;False;251;DEBUG;1;0;OBJECT;;False;1;FLOAT3;0
Node;AmplifyShaderEditor.RegisterLocalVarNode;251;3253.525,-13596.87;Inherit;False;DEBUG;-1;True;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.FunctionNode;292;2670.06,-13695.58;Inherit;False;Simple HUE;-1;;39;32abb5f0db087604486c2db83a2e817a;0;1;1;FLOAT;0;False;4;FLOAT3;6;FLOAT;7;FLOAT;5;FLOAT;8
Node;AmplifyShaderEditor.SimpleAddOpNode;295;1985.904,-13740.44;Inherit;False;3;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleRemainderNode;296;2242.904,-13690.44;Inherit;False;2;0;FLOAT;0;False;1;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.LerpOp;297;3024.045,-13342.22;Inherit;False;3;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;2;FLOAT;0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.RangedFloatNode;298;2812.846,-13266.99;Inherit;False;Constant;_Float11;Float 11;17;0;Create;True;0;0;0;False;0;False;0.5;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.NoiseGeneratorNode;264;1573.621,-13022.53;Inherit;False;Simplex2D;True;False;2;0;FLOAT2;0,0;False;1;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;266;1614.621,-13230.53;Inherit;False;Dots Pattern;-1;;40;7d8d5e315fd9002418fb41741d3a59cb;1,22,1;5;21;FLOAT2;0,0;False;3;FLOAT2;8,8;False;2;FLOAT;0.9;False;4;FLOAT;0.5;False;5;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.VoronoiNode;247;1549.029,-12874.51;Inherit;False;0;0;1;0;1;False;1;False;False;False;4;0;FLOAT2;0,0;False;1;FLOAT;0;False;2;FLOAT;2;False;3;FLOAT;0;False;3;FLOAT;0;FLOAT2;1;FLOAT2;2
Node;AmplifyShaderEditor.InstanceIdNode;299;2150.673,-13885.13;Inherit;False;False;0;1;INT;0
Node;AmplifyShaderEditor.LerpOp;300;2887.66,-13107.58;Inherit;False;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;301;2481.362,-13322.39;Inherit;True;Constant;_Float12;Float 12;17;0;Create;True;0;0;0;False;0;False;0.5;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.TransformPositionNode;294;1701.133,-13795.56;Inherit;False;Object;World;False;Fast;True;1;0;FLOAT3;0,0,0;False;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.ObjectScaleNode;302;1691.654,-13626.65;Inherit;False;False;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;303;2105.004,-13675.54;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0.1777777;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;72;3600.129,-7948.444;Float;False;True;-1;2;UnityEditor.ShaderGraphUnlitGUI;0;13;World;2992e84f91cbeb14eab234972e07ea9d;True;Forward;0;1;Forward;8;False;False;False;False;False;False;False;False;False;False;False;False;True;0;False;;False;True;0;False;;False;False;False;False;False;False;False;False;False;True;False;0;False;;255;False;;255;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;False;False;False;False;True;4;RenderPipeline=UniversalPipeline;RenderType=Opaque=RenderType;Queue=Geometry=Queue=0;UniversalMaterialType=Unlit;True;5;True;12;all;0;False;True;1;1;False;;0;False;;1;1;False;;0;False;;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;True;True;True;True;0;False;;False;False;False;False;False;False;False;True;False;0;False;;255;False;;255;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;False;True;1;False;;True;3;False;;True;True;0;False;;0;False;;True;1;LightMode=UniversalForwardOnly;False;False;0;;0;0;Standard;22;Surface;0;0;  Blend;0;0;Two Sided;1;638394846060462180;Forward Only;0;0;Cast Shadows;1;0;  Use Shadow Threshold;0;0;GPU Instancing;1;0;LOD CrossFade;1;0;Built-in Fog;1;0;DOTS Instancing;0;0;Meta Pass;0;0;Extra Pre Pass;0;0;Tessellation;0;0;  Phong;0;0;  Strength;0.5,False,;0;  Type;0;0;  Tess;16,False,;0;  Min;10,False,;0;  Max;25,False,;0;  Edge Length;16,False,;0;  Max Displacement;25,False,;0;Vertex Position,InvertActionOnDeselection;1;0;0;10;False;True;True;True;False;False;True;True;True;False;False;;False;0
Node;AmplifyShaderEditor.RangedFloatNode;305;3157.595,-7506.304;Inherit;False;Constant;_Float13;Float 13;18;0;Create;True;0;0;0;False;0;False;0.1;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;304;3104.412,-7668.281;Inherit;False;Property;_AlphaClip;AlphaClip;2;0;Create;True;0;0;0;False;0;False;0;1;1;True;;Toggle;2;Key0;Key1;Create;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;308;2946.54,-7782.175;Inherit;False;Constant;_Float14;Float 14;19;0;Create;True;0;0;0;False;0;False;1;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.TextureCoordinatesNode;306;2545.314,-7643.629;Inherit;False;0;-1;2;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT2;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SamplerNode;307;2768.54,-7670.175;Inherit;True;Property;_AlphaClipTexture;AlphaClipTexture;3;0;Create;True;0;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.TextureCoordinatesNode;97;-4427.244,-5538.41;Inherit;False;0;-1;2;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT2;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;309;-4159.824,-5460.56;Inherit;False;2;2;0;FLOAT2;0,0;False;1;FLOAT;0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.RangedFloatNode;310;-4383.824,-5361.56;Inherit;False;Property;_UVScale;UVScale;6;0;Create;True;0;0;0;False;0;False;1;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;288;-4446.73,-5232.477;Inherit;False;WorldUV_World;15;;42;4534d1524e6dfe64aa5c7393ec5283d7;0;3;80;FLOAT3;0,0,0;False;79;FLOAT3;0,0,0;False;65;FLOAT3;0,0,0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;311;-4113.824,-5301.56;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT2;0,0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.FresnelNode;258;2052.145,-13247.9;Inherit;False;Standard;WorldNormal;ViewDir;False;False;5;0;FLOAT3;0,0,1;False;4;FLOAT3;0,0,0;False;1;FLOAT;0;False;2;FLOAT;3;False;3;FLOAT;2;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RegisterLocalVarNode;196;4826.325,-5346.463;Inherit;False;AdditionalLight;-1;True;1;0;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.RegisterLocalVarNode;211;4402.897,-4802.152;Inherit;False;AdditionalLightLength;-1;True;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RegisterLocalVarNode;273;5413.56,-4765.273;Inherit;False;LightBoost;-1;True;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.ClampOpNode;312;5224.955,-4649.863;Inherit;False;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;40;False;1;FLOAT;0
...
WireConnection;271;0;211;0
WireConnection;271;1;272;0
WireConnection;274;0;271;0
WireConnection;274;1;275;0
WireConnection;214;0;276;0
WireConnection;276;0;98;0
WireConnection;276;1;277;0
WireConnection;289;0;250;0
WireConnection;289;1;291;0
WireConnection;291;0;290;0
WireConnection;291;1;290;0
WireConnection;251;0;297;0
WireConnection;292;1;296;0
WireConnection;295;0;302;1
WireConnection;295;1;302;2
WireConnection;295;2;302;3
WireConnection;296;0;303;0
WireConnection;297;0;292;6
WireConnection;297;1;300;0
WireConnection;297;2;298;0
WireConnection;264;0;246;0
WireConnection;266;21;269;0
WireConnection;247;0;246;0
WireConnection;300;0;253;0
WireConnection;300;1;301;0
WireConnection;300;2;301;0
WireConnection;303;0;295;0
WireConnection;96;1;309;0
WireConnection;96;0;311;0
WireConnection;72;2;289;0
WireConnection;72;3;304;0
WireConnection;72;4;305;0
WireConnection;304;1;308;0
WireConnection;304;0;307;1
WireConnection;307;1;306;0
WireConnection;309;0;97;0
WireConnection;309;1;310;0
WireConnection;311;0;310;0
WireConnection;311;1;288;0
WireConnection;196;0;243;0
WireConnection;211;0;43;0
WireConnection;273;0;312;0
WireConnection;312;0;274;0
ASEEND*/
//CHKSM=459F58EF71927C18A1A91E33591221440995114F